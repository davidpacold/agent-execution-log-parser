...
// API version - Cloudflare Worker environment variables are passed to the handler
...
export default {
  async fetch(request, env, ctx) {
    // Cloudflare Workers environment is available in env parameter
    // but not in global process.env
    const environment = env.ENVIRONMENT || 'development';
    
    // Log request information with environment context
    const requestInfo = extractRequestInfo(request);
    console.log(`[${environment}] Request received:`, requestInfo);
    ...
    ctx.waitUntil(
      (async () => {
        try {
          ...
          console.log(`[${environment}] Request completed in ${duration}ms`);
        } catch (error) {
